...
namespace mergeRicorsivo
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("Contatore = {0}", contatore);
        ...
        static void merge(int[] v, int[] b, int sinis, int dest)
        {
            int maxi;
            int i,j,k;

            maxi=(dest+sinis)/2;        //indice max del sottovettore sinistro
            k=sinis;
            i=sinis;
            j=maxi+1;
            if (sinis != maxi)
                merge(v,b,sinis,maxi);
            if (j != dest)
                merge(v,b,j,dest);
            do
            {
